...
import { productApi } from "../../db/data";
import ProductsNavBar from "../ProductNavBar/ProductnavBar";
import ProductsResults from "../ProductResult/ProductResult";
...
const Products: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (multipleFilters.brand.includes(true)) {
      if (multipleFilters.brand[0]) {
        tempProducts.forEach((product) => {
          if (product.productName === "Cleveland") filteredData.push(product);
        });
      }
      if (multipleFilters.brand[1]) {
        tempProducts.forEach((product) => {
          if (product.productName === "Atlanta") filteredData.push(product);
        });
      }
      filter1Applied = true;
    }
    ...
    if (multipleFilters.price[0]) {
      tempProducts.forEach((product) => {
        if (product.discountPrice && product.discountPrice < 500) {
          filteredData.push(product);
        }
      });
  ...
  return (
    <div className="products_page">
      ...
      <div className="filter_and_result_container">
        ...
        <ProductsResults products={productsList} />
      </div>
    </div>
  );
};
